 ...
 *
 * Adapted to TYPO3 by Dirk Wildt
 * Date: August 19, 2010
...
$rcmail_config['get_query'] = 'SELECT `descname`, `from_date`, `to_date`, `message`, `force_enabled`, `subject` FROM tx_postfix_autoresponder WHERE `email` = %u AND `hidden` = 0 AND `deleted` = 0 AND `pid` = '.$pid.' LIMIT 1';
...
$rcmail_config['insert_query'] = 'INSERT INTO tx_postfix_autoresponder(`pid`, `email`, `descname`, `from_date`, `to_date`, `message`, `enabled` , `force_enabled`, `subject`) VALUES ('.$pid.', %u, %d, %f, %t, %m, 0, %e, %s)';
...
$rcmail_config['update_query'] = 'UPDATE tx_postfix_autoresponder SET `descname` = %d, `from_date` = %f, `to_date` = %t, `message` = %m, `force_enabled` = %e, `subject` = %s WHERE `email` = %u AND `pid` = '.$pid.' LIMIT 1';